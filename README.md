...
**VETUSTUS Micro 0.2.0-beta.1** è la versione Android nativa e minimale di **VETUSTUS Script by VxD aka DEATHJ0KER**. IRC resta dietro le quinte: l'utente cerca, scarica, apre la Libreria e riproduce i contenuti senza usare un client IRC visibile.

## Stato Beta

Funzioni già operative e verificate sul campo:
...
VETUSTUS Micro non lascia più che un bot con una coda lunga blocchi tutti gli altri download. Può mantenere fino a **6 richieste IRC** in attesa contemporaneamente e assegna priorità DCC ai bot che diventano pronti per primi, con fino a **3 trasferimenti DCC** attivi in parallelo.
...
Questo permette, per esempio, di gestire reti che espongono un certificato TLS non coerente senza disattivare globalmente la sicurezza TLS.
...
```text
VETUSTUS-Micro-Android-0.2.0-beta.1-debug
```
...
9. Per gli archivi supportati estrae i media in modo selettivo e applica le difese contro path traversal, duplicati e decompression bomb.
